...
import { useChangePasswordController } from './change-password/useChangePasswordController';
import { ChangePasswordForm } from './change-password/ChangePasswordForm';
import { ChangePasswordSuccessState } from './change-password/ChangePasswordSuccessState';
  ...
  return (
    <View style={[styles.root, { backgroundColor: C.page }]}>
      <View style={[styles.backBtnWrap, { top: insets.top + GRID * 1.5 }]}>
        <Pressable style={styles.backBtn} onPress={() => ctrl.router.back()} hitSlop={12}>
        ...
        >
          <View style={styles.rail}>
            <View style={styles.titleBlock}>
              <Text style={styles.title}>{ctrl.success ? 'Password Updated' : 'Change Password'}</Text>
            ...
            >
              {ctrl.success ? (
                ...
                <ChangePasswordForm
                  currentPw={ctrl.currentPw} setCurrentPw={ctrl.setCurrentPw}
                  newPw={ctrl.newPw} setNewPw={ctrl.setNewPw}
                  confirmPw={ctrl.confirmPw} setConfirmPw={ctrl.setConfirmPw}
                  currentError={ctrl.currentError}
                  newError={ctrl.newError}
                  confirmError={ctrl.confirmError}
                  focused={ctrl.focused}
                  visible={ctrl.visible}
                  isSignedOut={ctrl.isSignedOut}
                  error={ctrl.error}
                  isValid={ctrl.isValid}
                  formDisabled={ctrl.formDisabled}
                  isPending={ctrl.isPending}
                  setFieldFocused={ctrl.setFieldFocused}
                  touch={ctrl.touch}
                  toggleVisible={ctrl.toggleVisible}
